...
char ap_ssid[32] = "ESP32_16CH_Smart_Switch";
...
long gmtOffset_sec = 28800;  // Philippines GMT+8
...
// Relay Configuration – expanded to 16 channels
#define NUM_RELAYS 16
const int relayPins[NUM_RELAYS] = {
  32, 33, 25, 26, 27, 14, 16, 17,   // original 8
  18, 19, 21, 22, 23,  4,  5, 13    // additional 8 (adjust to your hardware)
};
...
// System Configuration – includes AP settings and RTC persistence
struct SystemConfig {
  uint16_t magic;
  uint8_t version;
  char sta_ssid[32];
...
const unsigned long NTP_SYNC_INTERVAL = 1000; // 1 ms (consider changing to 3600000 for hourly sync)
...
<html>
<head>
    <title>ESP32 16-Channel Relay Timer</title>
...
<body>
    <div class="container">
        <div class="header">
            <h1>ESP32 16-Channel Relay Timer Switch</h1>
            <div class="nav">
...
<head>
    ...
    <style>
        ...
        .notification.success { background: #4CAF50; }
        .notification.error { background: #f44336; }
        .info { background: #e3f2fd; padding: 15px; border-radius: 3px; margin-bottom: 20px; color: #1976D2; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="card">
            <div class="info">
                <strong>Note:</strong> Changing AP settings will restart the device. 
                You'll need to reconnect using the new credentials.
            </div>
            <form onsubmit="saveAP(event)">
                <div class="form-group">
                    <label>AP SSID:</label>
                    <input type="text" id="ap_ssid" required minlength="1" maxlength="31">
                </div>
                <div class="form-group">
                    <label>AP Password:</label>
                    <input type="password" id="ap_password" minlength="8" maxlength="63">
                    <small style="color: #666;">Minimum 8 characters, leave blank for open network</small>
                </div>
    ...
    <script>
        ...
        function loadAP() {
            fetch('/api/ap')
                .then(response => response.json())
                .then(data => {
                    document.getElementById('ap_ssid').value = data.ap_ssid || 'ESP32_16CH_Smart_Switch';
                })
                .catch(error => {
        ...
        function saveAP(event) {
            ...
            .then(data => {
                if (data.success) {
                    showNotification('AP settings saved! Device will restart...');
                    setTimeout(() => {
                        window.location.href = '/';
                    }, 3000);
                } else {
                    showNotification(data.error || 'Failed to save AP settings', false);
                }
            })
            .catch(error => {
                console.error('Error:', error);
...
void setup() {
  ...
  if (!wifiConnected && sysConfig.last_ntp_sync_epoch > 1609459200) {
...
void loadConfiguration() {
  ...
  if (sysConfig.magic != EEPROM_MAGIC) {
    ...
    strcpy(sysConfig.ap_ssid, "ESP32_16CH_Smart_Switch");
    ...
    sysConfig.last_ntp_sync_epoch = 0;
    saveConfiguration();
  }
...
void handleGetRelays() {
  DynamicJsonDocument doc(8192); // increased for 16 relays
